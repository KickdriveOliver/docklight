...
' Converted to standard .txt/.vbs format - Original file name: Docklight-Arduino-MorseCode.pts
...
Sub sendMorseMessage(strMessage)
    For i = 1 To Len(strMessage)
        nextLetter = Mid(strMessage, i, 1)
        If nextLetter = " " Then
            ' "the space between words is seven units"
            DL.Pause 4 * unitDelayMSec  ' (we already had 3 units delay after completing the last letter)
        Else
            sendMorse (Mid(strMessage, i, 1))
            ' "the space between letters is three units"
            DL.Pause 2 * unitDelayMSec ' (we already had one unit delay within the For loop)
        End If
    Next
End Sub

Sub sendMorse(letter)
